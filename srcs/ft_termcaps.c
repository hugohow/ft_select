...
{
	fputs(tgetstr("us", NULL), stdin);
	ft_dprintf(0 ,"%-20s", arg->arg);
	fputs(tgetstr("me", NULL), stdin);
}
...
int go_to_col(int col)
{
    ...
    ft_term_get_window_size(&rows, &cols);
...
void ft_refresh(char *cmd, int *curr_index)
{
